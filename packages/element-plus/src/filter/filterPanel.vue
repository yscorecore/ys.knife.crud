...
/**
 * YsFilterPanel：搜索面板。聚合多个 YsFilterItem 子组件的 FilterInfo 为一个 AND 组合：
 *
 * - 默认插槽放多个 <ys-filter-item>，子组件经 provide/inject 自动注册到本面板
 * - 内置「搜索」「重置」按钮（showSearch/showReset 开关，默认 true）：
 *   - 搜索：emit('search', FilterInfo)，filter 是所有非空 item.filter 的 createAnd 聚合
 *   - 重置：清空全部 item 值并 emit('reset')
 * - showSearch=false 时不渲染内置搜索按钮，外部经 ref 取 panelApi.filter 自行触发
 *
 * 通用逻辑（items Map、filter computed、register/unregister）下沉到
 * @ys.knife.crud/vue 的 useFilterPanel；本组件只负责视图编排与事件分发。
 */
const props = defineProps({
  /** 是否渲染内置「搜索」按钮，默认 true */
...
const emit = defineEmits<{
  /** 搜索按钮点击：携带聚合后的 FilterInfo（全部为空时为 emptyFilter()） */
...
<template>
  <div class="yk-filter-panel">
    <div class="yk-filter-panel__items">
      <slot />
    </div>
