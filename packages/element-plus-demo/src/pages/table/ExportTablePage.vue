...
<template>
  <DemoPageLayout
    title="导出 Excel 表格"
    hint="表格不内置导出入口，由上方外部按钮调 tableRef.openExportDialog() 打开导出对话框：可选导出选中 / 当前页 / 所有数据（只有一页或无勾选列时对应选项自动隐藏，仅剩一个选项时不弹框直接导出）。导出列与界面所见一致（显隐 + 顺序 + 列宽）；「导出所有」会循环请求数据并显示进度条，可中途取消。"
    @back="$emit('back')"
  >
    <template #toolbar>
