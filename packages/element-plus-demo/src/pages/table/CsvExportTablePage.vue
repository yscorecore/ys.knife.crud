...
<template>
  <DemoPageLayout
    ...
    hint="showExportExcel 的导出格式由 exportorFunc 决定：组件只经 core 的 ExportApi 契约（renderHeader → renderRows → download/cancel）写数据，注入什么实现就产出什么格式。本页注入了一个手写的 CSV 导出器替代默认的 ExcelJS——外部按钮经 ref.openExportDialog() 打开入口，选「导出所有」后下载到的是 .csv 文件（记事本/Excel 可直接打开），取消导出则走 cancel 不产出文件。"
    @back="$emit('back')"
  >
    <template #toolbar>
      <el-button type="success" @click="tableRef?.openExportDialog()">⬇ 导出 CSV</el-button>
    </template>
