...
<template>
  <DemoPageLayout
    ...
    hint="10000 行数据 + 50ms 请求延迟，接口返回真实 totalCount：分页器直接显示精确的 Total 10000 与 500 个页码。全部功能开启——跨页勾选（翻页不丢，上方可「查看选中」）、行操作（查看恒可用；编辑在 age&lt;20 时禁用；id=1 受保护不显示删除，删除后显式 reload，总数实时减少）、列设置（localStorage 持久化，上方「⚙ 列设置」按钮调 openConfigDialog 打开）、导出 Excel（上方「⬇ 导出 Excel」按钮调 openExportDialog；选中 / 当前页 / 所有；导出所有按 500 条/次流式拉取 20 次，进度条显示精确百分比、分母「已加载 N / 10000」，并按当前拉取速率实时显示「预计剩余约 X 秒」）。"
    @back="$emit('back')"
  >
    <template #toolbar>
